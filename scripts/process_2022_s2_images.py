...
output_img_dir = os.path.join(os.getcwd(), "public", "images", "pyqs", "2022_s2")
...
for p_idx, page in enumerate(doc):
    ...
    for img_idx, img in enumerate(image_list):
        ...
        if w > 60 and h > 40:
            ...
            pub_id = f"nest_2022_s2_{os.path.splitext(fname)[0]}"
            try:
                res = cloudinary.uploader.upload(
                    fpath,
                    folder="nest_pyqs/2022_s2",
                    public_id=pub_id,
                    overwrite=True,
                    resource_type="image"
                )
...
with open(os.path.join(os.getcwd(), "scripts", "cloudinary_2022_s2_mapping.json"), "w", encoding="utf-8") as f:
    json.dump(cld_uploaded, f, indent=2)

print(f"\nUploaded {len(cld_uploaded)} diagrams to Cloudinary for NEST 2022 Session 2!")
