...
for fname in os.listdir(output_img_dir):
    if fname.endswith((".png", ".jpeg", ".jpg")):
        ...
        pub_id = f"nest_2019_{os.path.splitext(fname)[0]}"
        try:
            res = cloudinary.uploader.upload(
                fpath,
                folder="nest_pyqs/2019",
                public_id=pub_id,
                overwrite=True,
                resource_type="image"
            )
...
with open(os.path.join(os.getcwd(), "scripts", "cloudinary_2019_mapping.json"), "w", encoding="utf-8") as f:
    json.dump(cld_uploaded, f, indent=2)

print(f"\nUploaded {len(cld_uploaded)} diagrams to Cloudinary for NEST 2019!")
